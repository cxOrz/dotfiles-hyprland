...
DEPS=(
    ...
    # Hyprland ecosystem
    hyprpaper hyprlock hypridle wlopm hyprpicker hyprpolkitagent
    # Screenshot & Image
    grim slurp satty
    # Launcher
    rofi
...
echo -e "  ${DIM}Format: output name, mode, position, scale"
echo -e ""
echo -e "  Examples:"
echo -e "    Single (auto-detect):   ,preferred,auto,auto"
echo -e "    Specific:               HDMI-A-1, 1920x1080@60, 0x0, 1"
echo -e "    Dual:                   DP-1, 2560x1440@165, 0x0, 1"
echo -e "                            HDMI-A-1, 1920x1080@60, 2560x0, 1"
echo -e "    HiDPI:                  eDP-1, 2560x1600@120, 0x0, 1.5${NC}"
echo ""

MONITORS=()
echo -e "${CYAN} ?${NC} Enter monitor config ${DIM}(output,mode,position,scale - one per line, empty to finish)${NC}"
...
# monitors.lua
{
    ...
    echo "-- See: https://wiki.hyprland.org/Configuring/Basics/Monitors/"
    echo ""
    for entry in "${MONITORS[@]}"; do
        # Parse: output,mode,position,scale
        IFS=',' read -r output mode position scale <<< "$entry" || true
        # Trim whitespace
...
} > "$DOTFILES_DIR/.config/hypr/monitors.lua"
success "Updated monitors.lua"
...
# ═══════════════════════════════════════════════════════════════════
CONFIGS_TO_LINK=(dunst fcitx5 hypr kitty quickshell rofi waybar yazi)
BACKUP_NEEDED=false
